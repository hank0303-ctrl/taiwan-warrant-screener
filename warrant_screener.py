...
.ledger-extra.show{{display:table-row-group}}
...
function renderTracker() {{
  ...
  var filteredPositions = openPositions.filter(trackerPassPositionFilter);
  ...
  var realizedColor = summaryData.realized > 0 ? '#27ae60' : summaryData.realized < 0 ? '#e74c3c' : '#555';
  var unrealizedColor = summaryData.unrealized > 0 ? '#27ae60' : summaryData.unrealized < 0 ? '#e74c3c' : '#555';
  var monthlyPnl = 0;
  ...
  var positionHtml = filteredPositions.length ? filteredPositions.map(function(p) {{
    ...
    var txRows = p.transactions.map(function(tx) {{
      var gross = Number(tx.price || 0) * Number(tx.quantity || 0);
      return '<tr><td>'+trackerEscape(tx.trade_date)+'</td><td>'+trackerEscape(tx.side === 'sell' ? '賣出' : '買進')+'</td><td>'+trackerPrice(tx.price)+'</td><td>'+trackerMoney(tx.quantity)+'</td><td>'+trackerMoney(gross)+'</td><td>'+trackerMoney((tx.fee||0)+(tx.tax||0)+(tx.other_cost||0))+'</td></tr>';
    }}).join('');
    return '<div class="position-card">' +
      '<div class="position-head"><div><div class="position-title">'+trackerEscape(p.symbol)+' '+trackerEscape(p.name || '')+'｜'+trackerWarrantLabel(p.warrant_type)+'｜預設帳戶</div>' +
      '<div class="position-meta">'+trackerAssetLabel(p.asset_type)+'｜'+trackerSourceLabel(p.source)+'</div></div>' +
      '<div class="position-pnl" style="color:'+color+'">'+(p.total_pnl>0?'+':'')+trackerMoney(p.total_pnl)+'<br><span style="font-size:13px">'+(p.unrealized_return_pct>0?'+':'')+p.unrealized_return_pct.toFixed(2)+'%</span></div></div>' +
  ...
  var closedPositions = positions.filter(function(p){{return p.status === 'closed' && p.total_buy_quantity > 0;}});
  ...
  function ledgerRow(tx, withActions) {{
    ...
    return '<tr style="border-top:1px solid #f2f2f2">' +
      ...
      '<td style="padding:7px"><b>'+trackerEscape(tx.name || tx.symbol)+'</b><br><span style="font-size:11px;color:#777">'+trackerEscape(tx.symbol)+'｜預設帳戶</span></td>' +
...
<div class="card">
  ...
  <div class="tracker-grid" style="margin:14px 0 10px">
    <label>累計投入本金
