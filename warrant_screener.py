...
function addTradeRecord() {{
  ...
  var record = {{
    id: editId || Date.now(),
    action: document.getElementById('rt-action').value || 'buy',
...
function editTradeRecord(id) {{
  ...
  document.getElementById('rt-action').value = row.action || 'buy';
...
function renderTracker() {{
  ...
  var totalCost = 0, totalValue = 0, totalPnl = 0, realizedPnl = 0, wins = 0;
  ...
  rows.forEach(function(r) {{
    var action = r.action || 'buy';
    var entry = r.buy * r.qty;
    var exitValue = r.current * r.qty;
    var rowPnl = exitValue - entry;
    if (action === 'sell') {{
      totalCost -= exitValue;
      realizedPnl += rowPnl;
      totalPnl += rowPnl;
  ...
  var pnl = totalPnl;
  var roiBase = rows.reduce(function(sum, r){{ return sum + ((r.action || 'buy') === 'buy' ? r.buy * r.qty : 0); }}, 0);
  var roi = roiBase > 0 ? pnl / roiBase * 100 : 0;
  ...
  summary.innerHTML =
    ...
    '<div class="stat-box"><div class="stat-val" style="color:'+(realizedPnl>=0?'#27ae60':'#e74c3c')+'">'+(realizedPnl>=0?'+':'')+realizedPnl.toFixed(0)+'</div><div class="stat-lbl">已實現損益</div></div>' +
  ...
  body.innerHTML = rows.map(function(r) {{
    ...
    var action = r.action || 'buy';
    var actionLabel = action === 'sell' ? '賣出' : '買入';
    var actionColor = action === 'sell' ? '#e67e22' : '#1f5f99';
    ...
    return '<div style="border-top:1px solid #f0f0f0;padding:12px 0">' +
      ...
      '<div><b>'+code+' '+name+'</b> <span style="background:'+actionColor+';color:#fff;border-radius:4px;padding:1px 6px;font-size:11px;margin-left:4px">'+actionLabel+'</span> <span style="color:#777;font-size:12px">｜'+type+'｜'+source+'｜'+date+'</span><br>' +
      '<span style="font-size:12px;color:#777">成本 '+r.buy+'｜'+(action === 'sell' ? '賣出' : '目前')+' <input value="'+r.current+'" onchange="updateTradeCurrent('+r.id+', this.value)" style="width:76px;border:1px solid #ddd;border-radius:6px;padding:3px 6px">｜數量 '+r.qty+'｜'+(action === 'sell' ? '扣減成本 ' : '投入成本 ')+(action === 'sell' ? '-'+value.toFixed(0) : cost.toFixed(0))+' 元</span></div>' +
